...
import certificateData from "../../utils/constant/certificateData";
                ...
                <div className="group relative grid gap-4 pb-1 transition-all sm:grid-cols-8 sm:gap-8 md:gap-4 lg:hover:!opacity-100 lg:group-hover/list:opacity-50">
                  ...
                  <div className="z-10 sm:order-2 sm:col-span-5">
                    <h3>
                      ...
                      <span
                        className={
                          clicked === certificate.id ? "text-teal-300" : ""
                        }
                      >
                        {certificate.title} -{" "}
                    ...
                    {certificate.additionals[0].show && (
                      <Link
                        to={certificate.additionals[0].link}
                        target="_blank"
                        className="relative mt-2 inline-flex items-center text-sm font-medium text-slate-300 hover:text-teal-300 focus-visible:text-teal-300"
                      >
                        <FontAwesomeIcon
                          className="mr-1 h-3 w-3"
                          icon={certificate.additionals[0].icon}
                        />
                        <span>{certificate.additionals[0].text}</span>
                      </Link>
                    )}
                    <ul className="mt-2 flex flex-wrap">
                      {certificate.tag.map((tag, index) => (
