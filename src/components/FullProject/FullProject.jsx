...
import fullProjectsData from "../../utils/constant/fullProjectsData";
...
const fullProjects = () => {
  ...
  return (
    ...
    >
      {/* <NavTitle title={"allFullProjects"} /> */}

      <div>
        <ul className="group/list mt-8">
          {fullProjectsData.map((fullProjectss, index) => (
            <li
              key={index}
              className="mb-12"
              onClick={() => handleClick(fullProjectss.id)}
            >
              <motion.div
                  ...
                  <div className="z-10 sm:order-2 sm:col-span-5">
                    <h3>
                      ...
                      >
                        ...
                        <span
                          className={
                            clicked === fullProjectss.id ? "text-teal-300" : ""
                          }
                        >
                          {fullProjectss.title} -{" "}
                          <span className="inline-block">
                            {fullProjectss.category}
                            <FontAwesomeIcon
                    ...
                    {fullProjectss.additionals[0].show && (
                      <Link
                        to={fullProjectss.additionals[0].link}
                        target="_blank"
                        className="relative mt-2 inline-flex items-center text-sm font-medium text-slate-300 hover:text-teal-300 focus-visible:text-teal-300"
                      >
                        <FontAwesomeIcon
                          className="mr-1 h-3 w-3"
                          icon={fullProjectss.additionals[0].icon}
                        />
                        <span>{fullProjectss.additionals[0].text}</span>
                      </Link>
                    )}
                    <ul className="mt-2 flex flex-wrap">
                      {fullProjectss.tag.map((tag, index) => (
